...
func main() {
    ...
    //Earnings calendar
    earningsCalendar, _, err := finnhubClient.EarningsCalendar(context.Background()).From("2022-08-01").To("2022-08-25").Execute()
    fmt.Printf("%+v\n", earningsCalendar)
    ...
    news, _, err := finnhubClient.CompanyNews(context.Background()).Symbol("AAPL").From("2022-08-01").To("2022-08-11").Execute()
    if err != nil {
        fmt.Println(err)
    }
    fmt.Printf("%+v\n", news)
